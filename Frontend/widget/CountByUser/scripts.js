...
const endpoint = "http://localhost:3005/api/v1/tweets/counts";
...
function renderTweets(tweetList) {
    ...
    tweetList.forEach(tweet => {
        insertTable(tweet.usuario, tweet.count);
    });
}

function sortTweets(tweetList) {
...
function RefreshData() {
    fetch(endpoint).then(response => response.json()).then(tweetList => {
        ...
        if (tweetList.length > allTweets.length) {
            allTweets = sortTweets(tweetList);
